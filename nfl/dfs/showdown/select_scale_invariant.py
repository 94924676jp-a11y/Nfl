...
CAPS={'Tyrone Tracy Jr.':2,'Najee Harris':2}
NOCPT={'Tyrone Tracy Jr.','Najee Harris'}
...
for oi in order:
    ...
    ok=True
    for i in ids:
        nm=by(i)['name']; lim=CAPS.get(nm, 10 if i in TES else 14)
...
json.dump({'objective':'mean within-world lineup percentile (scale-invariant)',
 ...
 open('/tmp/claude-0/final_sel.json','w'), indent=1)
for l in out:
