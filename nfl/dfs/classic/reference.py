"""THE ACCEPTED CLASSIC BASELINE, frozen at commit fe25d98.

This file is a verbatim copy of `optimizer.py` as accepted. It exists for ONE
reason: every speedup must be proved to return the same answer, and "the same
answer as what?" needs a fixed, runnable definition rather than a memory of
how the code used to behave.

DO NOT OPTIMISE THIS FILE. Do not fix its comments, do not tidy it, do not
port improvements into it. Its value is that it does not change. If a
behavioural fix is genuinely needed here -- a real bug, not a speedup -- then
the baseline moves, and that is a decision recorded in a commit message
alongside the re-run equivalence evidence.

`nfl/tests/test_classic_equivalence.py` runs this and the live optimizer over
fixed fixtures and asserts identical objectives and identical lineups.
"""
...
SPEC_VERSION = 'dk-nfl-classic-optimizer-reference-fe25d98'
