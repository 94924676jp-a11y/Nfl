...
def slate_fits(season, week, players, role_priors=None, tiers=None,
               current_season=None, role_by_id=None, depth_rank=None,
    ...
    `current_season` SELECTS STAGE 6. Supplied, the class point forecast is
    built by `opportunity_centre` from evidence that includes this season and
    honours the governed role. Absent, it is the historical-only computation,
    byte-for-byte as before, so a baseline run stays reproducible and the two
    can be compared under exactly one change.
    """
    rp = role_priors or {}
    fits, states = {}, {}
    ...
    def _C(cls):
        if current_season is None:
            return P4.class_point_forecast(
                cls, season, week, players,
                role_prior=rp.get(cls), tiers=tiers)
        return P4.class_point_forecast_cs(
            cls, season, week, players, current_season=current_season,
            role_prior=rp.get(cls), tiers=tiers, role_by_id=role_by_id,
            depth_rank=depth_rank, declared_starters=declared_starters)

    for name, o in (
            ('p4c_params_targets', P4.params('targets', season)),
            ('p4c_params_carries', P4.params('carries', season)),
            ('C_targets', _C('targets')),
            ('C_carries', _C('carries')),
