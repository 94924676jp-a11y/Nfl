# Guard census — 89 guards, full columns

Generated by `nfl/tools/guard_reachability.py`; JSON companion
`GUARD_REACHABILITY.json`. **Re-run it rather than trusting these counts.**

`classification`, `load_bearing_proof` and test determinism are DECLARED in
the tool, defaulting to `NOT_ESTABLISHED`, because each is a judgement and a
tool that guessed them would manufacture the confidence this census exists to
strip out. Everything else is derived from the AST.
...
| INTERNAL_ONLY | 24 | | ANNOTATE | 13 | | ORPHANED_CONTROL | 2 |
| EXTERNALLY_CALLED | 35 | | DOWNGRADE | 6 | | TEST_NOT_RUNTIME | 1 |
|  |  | | VERDICT_REGISTERED | 5 | | SUPERSEDED | 1 |
|  |  | | NOTHING | 1 | | CANDIDATE_SCOPED | 1 |

## Guards with a load-bearing proof

The only ones that have earned LOAD_BEARING: a seeded bad state, the guard
reporting FAIL, and the **protected action demonstrably not happening**.

| guard | protects | proof | determinism |
|---|---|---|---|
| `assert_graded_row` | Every graded row proves how its actual arrived. No exception | `test_missing_is_not_zero.py` | DETERMINISTIC_MEASURED |
...
| `assert_shared_draws` | DK-scored players must be players the football layers emitte | `test_publication_refusal_is_load_bearing.py` | DETERMINISTIC_MEASURED |
...
| `assert_dropback_identity` | DOWNGRADE | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | — | **no** |
...
| `assert_no_outcome_shaped_inputs` | ANNOTATE | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | yes | yes |
| `assert_not_applicable_is_evidenced` | STOP | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | — | yes |
| `assert_not_dry_run` | ANNOTATE,DOWNGRADE | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | yes | **no** |
| `assert_not_mutated` | ANNOTATE | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | yes | **no** |
...
| `assert_publishable` | ANNOTATE | EXTERNALLY_CALLED | yes | — | NOT_ESTABLISHED | yes | yes |
...
| `assert_single_divergence` | STOP | INTERNAL_ONLY | yes | — | NOT_ESTABLISHED | yes | yes |
...
## The column that matters for VERIFIED

Only **27 of 89** guards have a caller referenced by any workflow,
runbook or doc. A guard reached only from a module nobody invokes is protected
by nothing, whatever its call graph says — which is why no defect fixed today
has moved past `FIX_IMPLEMENTED`.
