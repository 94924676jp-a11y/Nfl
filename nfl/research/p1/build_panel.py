...
def build_targets(season):
    ...
    # snap share, joined by name+team because snap_counts has no gsis_id
    snap_by_name = {}
    for (wk, tm, nm, pfr), r in snaps.items():
        snap_by_name[(wk, tm, (nm or '').strip())] = r
    hit = miss = 0
    for row in rows:
        r = snap_by_name.get((row['week'], row['team'],
...
if __name__ == '__main__':
    ...
    with open(f'{SP}/panel.csv', 'w', newline='') as fh:
        w = csv.DictWriter(fh, fieldnames=cols)
        w.writeheader()
        w.writerows(allrows)
    json.dump(diag, open(f'{SP}/panel_diag.json', 'w'), indent=1)
