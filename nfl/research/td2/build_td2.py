    ...
    with gzip.open(f'pbp{y}.csv.gz', 'rt') as fh:
        for r in csv.DictReader(fh):
            ...
            if rec and pas:
                ...
                if ptd:
                    p['rec_td'] += 1; T[tk]['rec_td'] += 1
                    if rz: p['rec_td_rz'] += 1
                    if in10: p['rec_td_in10'] += 1
                    if in5: p['rec_td_in5'] += 1
                    if g2g: p['rec_td_g2g'] += 1
            if rush and rus:
                T[tk]['carries'] += 1
                p = P[(y, wk, pos, rus)]
                p['carries'] += 1
                ...
                if rtd:
                    p['rush_td'] += 1; T[tk]['rush_td'] += 1
                    if rz: p['rush_td_rz'] += 1
                    if in10: p['rush_td_in10'] += 1
                    if in5: p['rush_td_in5'] += 1
                    if g2g: p['rush_td_g2g'] += 1
                    if i(r.get('qb_scramble')): p['rush_td_scramble'] += 1
                    else: p['rush_td_designed'] += 1
...
pickle.dump({'player': plyr, 'team': team, 'named_exclusions': dict(named)},
            open('td2.pkl', 'wb'))
print(f'wrote td2.pkl  {len(plyr)} player-games, {len(team)} team-games')
